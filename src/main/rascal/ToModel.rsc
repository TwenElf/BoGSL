...
MoveDef toMoveDef(Movement movementTree) {
  ...
  if(rule != []) return moveDef(moveName, steps, rule[0], tree= just(movementTree));
...
// Specific rule logic data structure conversion.
...
private RuleLogic toRuleLogic((RuleParts) `other player piece any`)      =  R_movement(R_oppponent(R_anyPiece()));
...
private RuleLogic toRuleLogic((RuleParts) `<RuleParts l> != <RuleParts r>`)  =  R_neq(toRuleLogic(l), toRuleLogic(r)); 
...
private RuleLogic toRuleLocation((RuleLocations) `{<RuleMovement movement>}`) =  R_location(R_pieceRef(unparse(id)), false);
