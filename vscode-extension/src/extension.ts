...
import {
  ...
  VSCodeUriResolverServer,
  LanguageParameter,
} from "@usethesource/rascal-vscode-dsl-lsp-server";
import { join } from "path";

export function activate(context: vscode.ExtensionContext) {
  // jar that contains the `pico-lsp` project
  const bogslLSPJar = `|jar+file://${context.extensionUri.path}/assets/jars/bogsl-lsp.jar!|`;
  const language = <LanguageParameter>{
    pathConfig: `pathConfig(srcs=[${bogslLSPJar}])`,
    name: "BoGSL",
    extensions: ["dsl"],
  ...
  const lsp = new ParameterizedLanguageServer(
    ...
    calcJarPath(context),
    true,
    "bogsl", // vscode language ID
    "BoGSL", // vscode language Title (visible in the right bottom corner)
    language,
  );
